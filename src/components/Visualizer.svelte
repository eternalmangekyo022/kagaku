<script lang='ts'>
	import { elements, mouseMode } from "../store";
...
<div class='h-full w-full flex overflow-x-hidden'>
	...
	<div class='h-full w-[85%] flex flex-col'>
		<div class='relative w-full h-[6%] flex justify-start gap-12 border-2 border-b-zinc-400 exception'>
			<div class='w-12 aspect-square grid place-items-center exception cursor-pointer' style='{$mouseMode === 'select' ? 'background: radial-gradient(#000, transparent 70%);' : ''}' on:click={() => mouseMode.set('select') } on:keyup>
			...
			<div class='h-full w-12 relative cursor-pointer exception' on:keyup on:click={() => mouseMode.set('bond')} style='{$mouseMode === 'bond' ? 'background: radial-gradient(rgba(0, 0, 0, 0.5), transparent 70%);' : ''}'>
				<svg class='exception'>
				...
				<BondComponent mouseUp={clearIntervals} mouseDown={({ clientX: x, clientY: y }) => {
						if($mouseMode === 'select') {
							...
							intervals.push(setInterval(() => {
								...
								const test = { start: { x: 0, y: 0 }, end: { x: 0, y: 0} }
								bond = { ...bond, pos }
							}, 10))
						}
				}} {bond}/>
