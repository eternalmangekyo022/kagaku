	...
	const mouseActions: { select: () => void, bond: () => void } = {
		...
		bond: () => {
			...
			bond.set({
				bondType: 'ionic',
				active: false,
				pos: [startLocation, startLocation]
			});

			intervals.push(setInterval(() => {
				bond.update(prev => ({ ...prev!, pos: [startLocation, [...mouse]] }));
			}, 5));
		}
	}
